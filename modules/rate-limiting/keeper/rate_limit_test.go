...
func (s *KeeperTestSuite) TestResetRateLimit() {
	...
	err := s.App.RatelimitKeeper.SetPendingSendPacket(s.Ctx, channelIdToRemove, sequence, denomToRemove)
	s.Require().NoError(err)
	err = s.App.RatelimitKeeper.SetPendingReceivePacket(s.Ctx, channelIdToRemove, sequence, denomToRemove)
	s.Require().NoError(err)
	err = s.App.RatelimitKeeper.SetPendingSendPacket(s.Ctx, channelIdToRemove, sequence, denomToKeep)
	s.Require().NoError(err)
	err = s.App.RatelimitKeeper.SetPendingReceivePacket(s.Ctx, channelIdToRemove, sequence, denomToKeep)
	s.Require().NoError(err)
	...
	found, err = s.App.RatelimitKeeper.CheckPacketSentDuringCurrentQuota(s.Ctx, channelIdToRemove, sequence, denomToRemove)
	s.Require().NoError(err)
	s.Require().False(found, "pending send packet should have been reset")
	found, err = s.App.RatelimitKeeper.CheckPacketReceivedDuringCurrentQuota(s.Ctx, channelIdToRemove, sequence, denomToRemove)
	s.Require().NoError(err)
	s.Require().False(found, "pending receive packet should have been reset")
	found, err = s.App.RatelimitKeeper.CheckPacketSentDuringCurrentQuota(s.Ctx, channelIdToRemove, sequence, denomToKeep)
	s.Require().NoError(err)
	s.Require().True(found, "pending send packet for different denom should not have been reset")
	found, err = s.App.RatelimitKeeper.CheckPacketReceivedDuringCurrentQuota(s.Ctx, channelIdToRemove, sequence, denomToKeep)
	s.Require().NoError(err)
