#!/bin/bash
# Run this script to quickly install, setup, and run the current version of simd without docker.
#
# Example:
# CHAIN_ID="local-1" HOME_DIR="$HOME/.simapp" TIMEOUT_COMMIT="500ms" CLEAN=true sh test_node.sh
# CHAIN_ID="local-2" HOME_DIR="~/.sim2" CLEAN=true RPC=36657 REST=2317 PROFF=6061 P2P=36656 GRPC=8090 GRPC_WEB=8091 ROSETTA=8081 TIMEOUT_COMMIT="500ms" sh scripts/test_node.sh
#
...
export MONIKER="local"
export KEYALGO="secp256k1"
export KEYRING=${KEYRING:-"os"}
export HOME_DIR=$(eval echo "${HOME_DIR:-"$HOME/.simapp"}")
...
from_scratch () {
  ...
  # sim1efd63aw40lxf3n4mhf7dzhjkr453axurv2zdzk
  ...
  # sim1hj5fveer5cjtn4wd6wstzugjfdxzl0xps73ftl
  ...
  BINARY init $MONIKER --chain-id $CHAIN_ID --default-denom token --home=$HOME_DIR
  ...
  # staking
  update_test_genesis '.app_state["staking"]["params"]["bond_denom"]="token"'  
  update_test_genesis '.app_state["staking"]["params"]["min_commission_rate"]="0.050000000000000000"'  
  # mint
  update_test_genesis '.app_state["mint"]["params"]["mint_denom"]="token"'  
  # crisis
  update_test_genesis '.app_state["crisis"]["constant_fee"]={"denom": "token","amount": "1000"}' 

  # PFM
  update_test_genesis '.app_state["packetfowardmiddleware"]["params"]["fee_percentage"]="0.000000000000000000"'

  # Allocate genesis accounts
  BINARY genesis add-genesis-account $KEY 10000000token,1000utest --keyring-backend $KEYRING --home=$HOME_DIR
  BINARY genesis add-genesis-account $KEY2 1000000token,1000utest --keyring-backend $KEYRING  

  BINARY genesis gentx $KEY 1000000token --keyring-backend $KEYRING --chain-id $CHAIN_ID
